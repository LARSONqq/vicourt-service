...
import { recurringTaskCopy, templateStateLabel, templateStatuses, templateWorkspaceHref } from "@/lib/taskTemplateWorkspace";
...
export default async function TemplatesPage({ searchParams }: { searchParams: Promise<TaskWorkspaceQuery> }) {
  ...
  return <div className="min-w-0 space-y-5">
    ...
    <header className="flex flex-wrap items-start justify-between gap-3">
      <div><h1 className="text-2xl font-bold">{recurringTaskCopy.title}</h1><p className="mt-1 text-sm text-gray-500">{recurringTaskCopy.description}</p><p className="mt-2 text-xs text-gray-500">Активна серія прив’язана до об’єкта або техніки. Шаблон зберігає налаштування для подальшого запуску.</p></div>
      <Link href="/tasks/templates/new" className="min-h-11 rounded-lg bg-green-700 px-4 py-3 text-sm text-white">{recurringTaskCopy.create}</Link>
    </header>
    ...
    {templates.length === 0 && <div className="rounded-xl border bg-white p-6 text-gray-500">{filters.q || filters.status !== "all" ? "За цими умовами серій або шаблонів немає. Змініть пошук чи скиньте фільтри." : "Повторюваних задач поки немає. Створіть серію або збережіть її налаштування як шаблон."}</div>}
    ...
    <nav aria-label="Сторінки повторюваних задач" className="flex flex-wrap items-center justify-between gap-3 text-sm">
