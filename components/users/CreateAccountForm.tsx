...
export default function CreateAccountForm({ kind }: { kind: "internal" | "client" }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Newest clients are on page 1. Retain the object context, not a stale
        // name filter/page that can hide a successfully created client.
        if (kind === "client" && (searchParams.has("q") || searchParams.has("page"))) {
          const params = new URLSearchParams(searchParams.toString());
          params.delete("q"); params.delete("page");
          const query = params.toString();
          router.replace(`/users/clients${query ? `?${query}` : ""}`);
        }
        router.refresh();
      }
  ...
  return <section className="min-w-0 space-y-3 rounded-xl border bg-white p-4">
    ...
    {result && <p role={result.ok ? "status" : "alert"} className={`text-sm ${result.ok ? "text-green-700" : "text-red-700"}`}>{result.message}</p>}
    {open && <form onSubmit={submit} className="grid min-w-0 gap-3 sm:max-w-lg">
      ...
      <label className="text-sm">Пароль (12–128 символів)<input name="password" type="password" autoComplete="new-password" required minLength={12} maxLength={128} className="mt-1 min-h-11 w-full rounded-lg border px-3" /></label>
      ...
      <button type="submit" disabled={pending} className="min-h-11 rounded-lg bg-green-700 px-4 py-2 text-white disabled:opacity-50">{pending ? "Створення…" : "Створити"}</button>
    </form>}
  </section>;
}
