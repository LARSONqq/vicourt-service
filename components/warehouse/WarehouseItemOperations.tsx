...
export default function WarehouseItemOperations({ itemId, name, unit, currency, canAdjust }: Props) {
  ...
  function open(nextOperation: WarehouseOperation) {
  ...
  return (
    <section className="min-w-0 rounded-xl border bg-white p-4 sm:p-5">
      <h2 className="mb-3 font-semibold text-gray-900">Операції з матеріалом</h2>
      <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
        {(Object.keys(labels) as WarehouseOperation[]).filter((value) => value !== "adjustment" || canAdjust).map((value) => (
          <button key={value} type="button" disabled={busy} onClick={() => open(value)} className={`${buttonClass} border-green-600 text-green-700 hover:bg-green-50`}>
            {buttons[value]}
          </button>
        ))}
      </div>
      ...
      <dialog ref={dialog} aria-labelledby="warehouse-operation-title" onCancel={(event) => {
        event.preventDefault();
        if (!busy) close();
        ...
        <div className="mb-4 flex min-w-0 items-start justify-between gap-3">
          <div className="min-w-0">
            <h2 id="warehouse-operation-title" className="text-xl font-semibold">{operation && labels[operation]}</h2>
          ...
          : !options ? <button type="button" disabled={busy} onClick={() => operation && void loadOptions(operation)} className={buttonClass}>Повторити завантаження</button>
            : <>
              ...
              {operation === "receipt" && <div className="mb-4 space-y-3">
                ...
                <button type="button" disabled={busy} onClick={() => { setPlanning(!planning); setQuantity(""); setError(""); }} className={`${buttonClass} w-full`}>
              ...
              <form onSubmit={submit} className="min-w-0 space-y-4">
                <fieldset disabled={busy} className="min-w-0 space-y-4 disabled:opacity-60">
                  ...
                  {(planning || operation !== "receipt") && <label className="block text-sm font-medium">Кількість, {unit}
                    <input type="number" name="quantity" inputMode="decimal" min="0.01" step="0.01" max={planning ? undefined : maxQuantity} value={quantity} onChange={(event) => setQuantity(event.target.value)} required className={fieldClass} />
                  </label>}
                  {operation === "adjustment" && <>
                  ...
                  {planning && <>
                    ...
                    <label className="block text-sm font-medium">Постачальник (необов’язково)<input name="supplier" maxLength={500} className={fieldClass} /></label>
